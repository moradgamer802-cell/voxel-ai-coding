#!/usr/bin/env bash
# ============================================================
#  BOOMCODE — one-line installer
#  Install: curl -fsSL https://raw.githubusercontent.com/
#           zyvo9/boomcode/main/install.sh | bash
#      or:  pip install boomcode && boomcode        (same installer)
#
#  When run by the pip bootstrap, the BOOMCODE layer is already on
#  disk at $BOOMCODE_BOOT and is used instead of downloading it.
#
...
setup_env() {
    ...
    # one-time storage permission (Termux): without the Android grant,
    # /sdcard is unreachable and every project folder would fall back to
    # ~/boomcode. termux-setup-storage pops the Allow dialog once; skip
    # silently when storage already works or the tool is unavailable.
    if [ "$ENV_KIND" = "termux" ] && command -v termux-setup-storage >/dev/null 2>&1; then
        if ! ls /storage/emulated/0 >/dev/null 2>&1; then
            echo
            status "storage permission - tap Allow on the Android dialog"
    ...
    case "$ENV_KIND" in
        ...
        *)
            ...
            BIN_DIR="${BOOMCODE_BIN_DIR:-$HOME/.local/bin}"
            STAMP_DIR="$HOME/.local/libexec/boomcode"; LIB_DIR="$HOME/.local/lib/boomcode"
            mkdir -p "$BIN_DIR" "$STAMP_DIR" "$LIB_DIR" 2>/dev/null || {
                # read-only or exotic $HOME — keep going in a writable place
                BIN_DIR="${TMPDIR:-/tmp}/boomcode/bin"; STAMP_DIR="${TMPDIR:-/tmp}/boomcode/libexec"
                LIB_DIR="${TMPDIR:-/tmp}/boomcode/lib"
    ...
    if [ -n "$BOOMCODE_FORCE_NODE" ]; then
        NATIVE_CORE=0
        status "forced Node core (BOOMCODE_FORCE_NODE)"
        bump 2
        return
    fi
...
setup_core_npm() { # architecture-neutral fallback (32-bit ARM, x86, odd ROMs)
    ...
    printf 'npm' > "$STAMP_DIR/boomcode-core-kind"
...
setup_core() {
    ...
    if [ "$ENV_KIND" = "termux" ]; then
        ...
        installed=""
        [ -f "$STAMP_DIR/boomcode-core-version" ] && installed="$(tr -d '[:space:]' < "$STAMP_DIR/boomcode-core-version" 2>/dev/null)"
        ...
        [ -n "$tag" ] && printf '%s' "$tag" > "$STAMP_DIR/boomcode-core-version"
        printf 'native' > "$STAMP_DIR/boomcode-core-kind"
...
# ------------------------------------------------------------
# Step 4 — BOOMCODE layer (wrapper, config, skills, commands)
# ------------------------------------------------------------
setup_layer() {
    # pip/bootstrap route: layer already on disk — skip network fetch
    if [ -n "$BOOMCODE_BOOT" ] && [ -d "$BOOMCODE_BOOT/scripts" ] && [ -d "$BOOMCODE_BOOT/config" ]; then
        status "boomcode layer ready (pip bootstrap)"
        rm -rf "$TMP"/* 2>/dev/null || true
        cp -a "$BOOMCODE_BOOT/." "$TMP/"
        bump 75
    else
    status "downloading boomcode layer"
    ...
    if [ ! -d "$TMP/config" ]; then
        run_bg "fetching boomcode layer (git)" bash -c \
            "git clone --depth 1 -q 'https://github.com/$GH_REPO.git' '$TMP/src' && mv '$TMP/src'/* '$TMP/' && rm -rf '$TMP/src'" \
            || fatal "couldn't download the BOOMCODE layer" "Check your internet and rerun."
    fi
    bump 75
    fi

    # wrapper + helpers
    install_script "$TMP/scripts/boomcode" "$BIN_DIR/boomcode"
    [ -f "$TMP/scripts/boomcode-menu" ]     && install_script "$TMP/scripts/boomcode-menu" "$BIN_DIR/boomcode-menu"
    [ -f "$TMP/scripts/boomcode-browser" ]  && install_script "$TMP/scripts/boomcode-browser" "$BIN_DIR/boomcode-browser"
    [ -f "$TMP/scripts/boomcode-vision" ]   && install_script "$TMP/scripts/boomcode-vision" "$BIN_DIR/boomcode-vision"
    [ -f "$TMP/scripts/boomcode-uninstall" ] && install_script "$TMP/scripts/boomcode-uninstall" "$BIN_DIR/boomcode-uninstall"
...
PYC
    ...
    if ! grep -q 'BOOMCODE PATH' "$rc" 2>/dev/null; then
        printf '# BOOMCODE PATH\ncase ":$PATH:" in *":%s:"*) ;; *) export PATH="%s:$PATH";; esac\n' \
            "$BIN_DIR" "$BIN_DIR" >> "$rc"
    fi
...
brand_logo() { # put the BOOMCODE brand logo into the core binary
    # patch-brand.py replaces the "OPENCODE" pixel-art wordmark with the
    # "BOOMCODE" design, same byte length (layout stays intact — no crash),
    ...
    if ! run "$py" "$LIB_DIR/patch-brand.py" "$STAMP_DIR/opencode.bin" /dev/null --verify; then
        ...
        rm -f "$STAMP_DIR/boomcode-core-version"
        if ! setup_core; then
...
try:
    ...
    repo = {}
# these always come from the repo — BOOMCODE's core defaults must win
...
for k, v in repo.items():
    if k in FORCE or k not in user:
        user[k] = v
# model defaults are intentionally NOT forced — BOOMCODE uses whatever model
...
verify_install() {
    status "verifying"
    bump 97
    if [ ! -x "$BIN_DIR/boomcode" ]; then
        fatal "wrapper missing at $BIN_DIR/boomcode" "Rerun the installer."
    fi
    ...
    VERSION="$("$BIN_DIR/boomcode" --version 2>&1 || true)"
    bump 99
    STATUS="verify ok"
    draw
}
...
if [ "${1:-}" = "uninstall" ]; then
    ...
    if [ -n "$BOOMCODE_BOOT" ] && [ -f "$BOOMCODE_BOOT/scripts/boomcode-uninstall" ]; then
        exec sh "$BOOMCODE_BOOT/scripts/boomcode-uninstall" "$@"
    fi
    SCRIPT_DIR="$(cd "$(dirname "$0")" 2>/dev/null && pwd || echo "$PWD")"
    if [ -f "$SCRIPT_DIR/scripts/boomcode-uninstall" ]; then
        exec sh "$SCRIPT_DIR/scripts/boomcode-uninstall" "$@"
    fi
    curl -fsSL --retry 2 --connect-timeout 15 --max-time 60 \
        -o "$TMP/boomcode-uninstall" "https://raw.githubusercontent.com/$GH_REPO/main/scripts/boomcode-uninstall" \
        && exec sh "$TMP/boomcode-uninstall" "$@"
...
printf "  ${C_C}${C_B} ╚═════╝  ╚═════╝  ╚═════╝  ╚══════╝${C_N}\n"
printf "\n"
printf "  ${C_G}${C_B}⚡ BOOMCODE READY${C_N}   ${C_D}%s · %s · install time ~%ss${C_N}\n" "$ENV_KIND" "$ARCH" "$SECONDS"
printf "  ${C_B}boomcode${C_N}              start the AI (full-power)\n"
printf "  ${C_B}boomcode preview${C_N}      open a page in the browser\n"
printf "  ${C_B}boomcode update${C_N}       delta update\n"
printf "  ${C_B}boomcode session${C_N}      session menu (↑↓ to pick)\n"
printf "  ${C_B}boomcode doctor${C_N}       health check\n"
printf "  ${C_B}boomcode backup${C_N}       backup config + projects\n"
printf "  ${C_B}boomcode uninstall${C_N}    remove BOOMCODE (keeps projects)\n"
if [ -n "$BOOMCODE_BOOT" ]; then
    printf "  ${C_D}installed via pip — update layer with:${C_N} ${C_B}boomcode install${C_N}\n"
fi
printf "\n  ${C_D}open a new shell, then run:${C_N} ${C_B}boomcode${C_N}\n\n"
